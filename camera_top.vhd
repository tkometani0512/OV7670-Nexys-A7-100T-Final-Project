...
-- Top file basically connects everything together while also covering anything that could cause a problem. It crops the image out/frames it. It also compensates for on board bram timing problems.
ENTITY camera_top IS
    PORT (
        ...
        -- It is good to note that there are two more pins on the camera which correspond to reset and pwdn. These can be configured in the VHDL and xdc files but for testing purposes and sake of time and effort, it is simpler to just use a bread board and tie down the RST pin to power and the PWDN pin to ground. I was researching and it may be more stable and consistent to have the fpga send all this stuff itself, but for the sake of time in the project I have opted for the bread board method.
    );
END camera_top;
